...
static int lpm009m360a_blanking_on(const struct device *dev)
{
	return lpm009m360a_sleep(dev);
}
...
static int lpm009m360a_read(const struct device *dev,
							const uint16_t x,
							const uint16_t y,
...
static int lpm009m360a_write(const struct device *dev,
							 const uint16_t x,
							 const uint16_t y,
							 const struct display_buffer_descriptor *desc,
							 const void *buf)
		...
		if (x % 2)
		{
			for (uint8_t i = 0; i < desc->height; i++)
			{
				for (uint8_t j = 0; j < desc->width;)
				{
					...
					data->buf[(y + i) * 36 + (x + j) / 2] |= (0x0f & (RGB565_RGB111(source_buf[(i * desc->width) + j])));
					j++;
					if (j < desc->width)
					{
						data->buf[(y + i) * 36 + (x + j) / 2] &= 0x0f;
						data->buf[(y + i) * 36 + (x + j) / 2] |= (0x0f & (RGB565_RGB111(source_buf[i * desc->width + j]))) << 4;
						j++;
					}
				}
			}
...
static void *lpm009m360a_get_framebuffer(const struct device *dev)
{
	return NULL;
}

static int lpm009m360a_set_brightness(const struct device *dev,
									  const uint8_t brightness)
{
	return -ENOTSUP;
}

static int lpm009m360a_set_contrast(const struct device *dev,
...
static int lpm009m360a_init(const struct device *dev)
{
	...
	ret = gpio_pin_configure_dt(&config->extcomin,
...
static int lpm009m360a_pm_action(const struct device *dev,
...
#define LPM009M360A_INIT(inst)                                                             \
	...
	DEVICE_DT_INST_DEFINE(inst,                                                            \
						  ...
						  &lpm009m360a_data_##inst,                                        \
						  &lpm009m360a_config_##inst,                                      \
						  POST_KERNEL,                                                     \
